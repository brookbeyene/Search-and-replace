...
def findandreplace(find, replace, string):
    ...
    if not string:
        ...
        print("The string is %s " % ( "None"))
        print("This is the end of search"
              + "\n")
    ...
    if string[:len(find)] == find:
        ...
        print('The string to find is ' + find
              + ' within ' + string +' is found')

        return replace + findandreplace(find, replace, string[len(find):])

    else:

        print('After looking for ' + find
              + ' in string is not found, inside of ' + string)
